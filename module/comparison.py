...
class DataStructureComparison:
    def __init__(self):
        self.array_data = []
        self.btree = BTree(t=3)  #minimum degree
        self.data_columns = None  
        self.array_lock = threading.Lock()  
        self.saved_trees_dir = "saved_trees"
        if not os.path.exists(self.saved_trees_dir):
            os.makedirs(self.saved_trees_dir)
    ...
    def load_data(self, file_path):
        ...
        for _, row in df.iterrows():
            ...
            key = record[self.data_columns[0]]  
    ...
    def save_btree(self, name):
        """Save current B-tree structure with a given name"""
        if not name.endswith('.json'):
            name += '.json'
        filename = os.path.join(self.saved_trees_dir, name)
        self.btree.save_to_file(filename)
        return filename

    def load_btree(self, name):
        """Load B-tree structure from a saved file"""
        if not name.endswith('.json'):
            name += '.json'
        filename = os.path.join(self.saved_trees_dir, name)
        if not os.path.exists(filename):
            raise FileNotFoundError(f"Không tìm thấy file cây B-tree: {filename}")
        self.btree = BTree.load_from_file(filename)
        return True

    def list_saved_trees(self):
        """List all saved B-tree structures"""
        if not os.path.exists(self.saved_trees_dir):
            return []
        return [f for f in os.listdir(self.saved_trees_dir) if f.endswith('.json')] 